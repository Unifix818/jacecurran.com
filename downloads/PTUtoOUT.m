function PTUtoOUT % Read PicoQuant Unified TTTR Files
...
function GotPhoton(TimeTag, Channel, DTime)
  ...
  % Alright, so here's the 411. This code works extremely well, and can
  % give you any relevant data you need. Overflows are already taken care
  % of by the built in function so don't worry about that. Issues:
  % 1) The original printing functions gave you a lot of unneccesary
  % information. At least as far as our experiments are concerned, you only
  % ever need DTime for T3, or Channel Number and TimeTag for T2.
  % 2) The original printing functions' formatting is broken. Somewhere
  % along the line, (is it the %1x, not sure) the latest versions of MATLAB
  % can't parse the formatting character array. Two birds, one stone, just
  % print only the needed information and do everything in the companion
  % code, OUTtoDAT.
  if(isT2)
        % fprintf(fpout,'%i CHN %1x %i %8.0lf\n', RecNum, Channel, TimeTag, (TimeTag * MeasDesc_GlobalResolution * 1e12));
        fprintf(fpout,'CHN %i %i \n', Channel, TimeTag);
  else
